...
const mockReminders = [
  {
    ...
    daysLate: 75,
    nextAction: "Avocat si non regle J+15",
    channel: "Courrier",
  },
  {
    id: "2",
    ...
    daysLate: 45,
    nextAction: "Mise en demeure le 28/03",
    channel: "Email + SMS",
  },
  {
    id: "3",
    tenant: "CARREFOUR Market",
    amount: 1_800_000,
    step: 1,
    label: "Rappel courtois",
    daysLate: 18,
    nextAction: "Relance 2 le 22/03",
    channel: "Email",
  },
  {
    id: "4",
    tenant: "Kiosque Fleurs",
    amount: 70_000,
    step: 1,
    label: "Rappel courtois",
    daysLate: 8,
    nextAction: "Relance 2 le 23/03",
    channel: "Email",
  },
];
...
const mockFranchises = [
  {
    id: "1",
    tenant: "SEPHORA CI",
    type: "Franchise totale",
    start: "01/04/2026",
    end: "30/06/2026",
    rent: "0 FCFA",
    charges: "350 000 FCFA/mois",
  },
  {
    id: "2",
    tenant: "Burger King",
    type: "Loyer progressif",
    start: "01/05/2026",
    end: "31/10/2026",
    rent: "1 000 000 → 4 800 000",
    charges: "Normales",
  },
];
...
const mockRevisions = [
  ...
  {
    id: "3",
    tenant: "Banque Atlantique",
    currentRent: 5_500_000,
    newRent: 5_665_000,
    rate: "+3.0%",
    type: "Taux fixe",
    date: "01/07/2026",
...
const mockVacants = [
  {
    ...
    zone: "Zone B",
    surface: 85,
    vacantSince: "15/01/2026",
    days: 62,
    lostRent: 1_890_000,
    pipeline: true,
    returnEst: "01/05/2026",
  },
  {
    id: "2",
    unit: "A-12",
    zone: "Zone A",
    surface: 120,
    vacantSince: "01/07/2026",
    days: 0,
    lostRent: 2_478_000,
    pipeline: false,
    returnEst: "01/10/2026",
  },
];
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-lg font-bold">
                  {
                    mockApplications.filter((a) => a.status !== "converted")
                      .length
                  }
                </p>
                <p className="text-[10px] text-muted-foreground">
        ...
        <Card>
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-lg font-bold">
                  {mockRevisions.filter((r) => r.status !== "applied").length}
                </p>
                <p className="text-[10px] text-muted-foreground">
                  Revisions a valider
                </p>
      ...
      <Tabs defaultValue="reminders">
        <TabsList className="grid w-full grid-cols-6">
          ...
          <TabsTrigger value="lease-end" className="text-xs">
            <DoorOpen className="h-3 w-3 mr-1" />
            Fins bail
          </TabsTrigger>
          <TabsTrigger value="franchises" className="text-xs">
          ...
          <TabsTrigger value="vacant" className="text-xs">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">
                Relances en cours —{" "}
                {formatFrancs(mockReminders.reduce((s, r) => s + r.amount, 0))}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">Montant du</TableHead>
                    <TableHead className="text-center">Etape</TableHead>
                  ...
                  {mockReminders.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right text-red-600 font-medium">
                        {formatFrancs(r.amount)}
                      </TableCell>
                      <TableCell className="text-center">
                        <Badge
                          className={cn("text-[10px]", stepColors[r.step])}
                        >
                          {r.label}
                        </Badge>
                      </TableCell>
        ...
        <TabsContent value="applications" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Candidatures locataires</CardTitle>
              <CardDescription>
                Evaluation, validation, conversion
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Local vise</TableHead>
                    <TableHead className="text-right">Loyer propose</TableHead>
                ...
                <TableBody>
                  {mockApplications.map((a) => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell className="text-center">
                        <Badge
                          className={cn("text-[10px]", statusColors[a.status])}
                        >
                          {a.status}
                        </Badge>
                      </TableCell>
        ...
        <TabsContent value="lease-end" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Fins de bail en cours</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">Depot garantie</TableHead>
                    <TableHead className="text-center">Statut</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {mockLeaseEndings.map((l) => (
                    <TableRow key={l.id}>
                      ...
                      <TableCell className="text-right">
                        {formatFrancs(l.deposit)}
                      </TableCell>
                      <TableCell className="text-center">
                        <Badge
                          variant={
                            l.status === "completed" ? "success" : "secondary"
                          }
                          className="text-[10px]"
                        >
                          {l.status}
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Loyer pendant franchise</TableHead>
            ...
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Locataire</TableHead>
                    <TableHead className="text-right">Loyer actuel</TableHead>
                    <TableHead className="text-right">Nouveau loyer</TableHead>
                ...
                <TableBody>
                  {mockRevisions.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell className="font-medium">{r.tenant}</TableCell>
                      <TableCell className="text-right">
                        {formatFrancs(r.currentRent)}
                      </TableCell>
                      <TableCell className="text-right font-medium">
                        {formatFrancs(r.newRent)}
                      </TableCell>
        ...
        <TabsContent value="vacant" className="mt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manque a gagner mensuel :{" "}
                {formatFrancs(mockVacants.reduce((s, v) => s + v.lostRent, 0))}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Vacant depuis</TableHead>
                    <TableHead className="text-right">
                      Loyer perdu/mois
                    </TableHead>
                    <TableHead className="text-center">Pipeline</TableHead>
                    <TableHead>Retour estime</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {mockVacants.map((v) => (
