...
export function RentIndexation({
  ...
  const [selectedType, setSelectedType] = useState<IndexationType>(
    lease.indexation_type,
  );

  // Projected next rent after indexation
  const projectedRent = Math.round(
    lease.monthly_rent_ht * (1 + lease.indexation_rate / 100),
  );
        ...
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <div>
              <p className="text-sm text-muted-foreground">
                {t("indexation.reference", "Reference")}
              </p>
              <p className="font-medium">{lease.lease_reference}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">
                {t("indexation.current_rent", "Current Rent HT")}
              </p>
            ...
            <div>
              <p className="text-sm text-muted-foreground">
                {t("indexation.period", "Lease Period")}
              </p>
              <p className="font-medium">
          ...
          <div className="mt-4 rounded-lg border p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div>
                <p className="text-sm text-muted-foreground">
                  {t("indexation.rate", "Current Rate")}
                </p>
                <p className="font-medium">{lease.indexation_rate}%</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">
                  {t("indexation.last_applied", "Last Applied")}
                </p>
                <p className="font-medium">
                  {formatDate(lease.indexation_date)}
                </p>
              </div>
            </div>
          </div>
      ...
      <Card className="border-blue-200 bg-blue-50/50">
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div>
              <p className="text-sm text-muted-foreground">
                {t("indexation.next_date", "Next Date")}
              </p>
              <Badge variant="outline" className="mt-1">
                {formatDate(lease.next_indexation_date)}
              </Badge>
            </div>
            <div className="flex items-center gap-3">
              <div className="text-center">
                <p className="text-sm text-muted-foreground">
                  {t("indexation.current", "Current")}
                </p>
                <p className="font-medium">
                  {formatFrancs(lease.monthly_rent_ht)}
                </p>
              ...
              <div className="text-center">
                <p className="text-sm text-muted-foreground">
                  {t("indexation.projected", "Projected")}
                </p>
                <p className="font-medium text-blue-700">
                  {formatFrancs(projectedRent)}
                </p>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          {isLoadingHistory ? (
            ...
            <p className="text-sm text-muted-foreground py-4 text-center">
              {t(
                "indexation.no_history",
                "No indexation history available yet.",
              )}
            </p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">
                    {t("indexation.old_amount", "Old Rent HT")}
                  </TableHead>
                  <TableHead className="text-right">
                    {t("indexation.new_amount", "New Rent HT")}
                  </TableHead>
                  <TableHead className="text-right">
                    {t("indexation.rate_applied", "Rate Applied")}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {sortedHistory.map((entry) => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell className="text-right">
                      {formatFrancs(entry.old_rent_ht)}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatFrancs(entry.new_rent_ht)}
                    </TableCell>
                    <TableCell className="text-right">
