...
const MOCK_RECEIVABLES: AgedReceivable[] = [
  ...
  },
  {
    id: "5",
    zone: "Zone A",
    counterparty: "Local A-12 (vacant)",
    not_due: 0,
    days_1_30: 0,
    days_31_60: 0,
    days_61_90: 0,
  ...
  // Zone B
  {
    id: "6",
    zone: "Zone B",
    counterparty: "Banque Atlantique",
    not_due: 5_500_000,
    days_1_30: 0,
    days_31_60: 0,
    days_61_90: 0,
  ...
  // Autres
  {
    id: "11",
    zone: "Autres",
    counterparty: "Regularisation charges",
    not_due: 0,
    days_1_30: 2_400_000,
    days_31_60: 0,
    days_61_90: 0,
  ...
  },
  {
    id: "12",
    zone: "Autres",
    counterparty: "Droits entree (att.)",
    not_due: 5_000_000,
    days_1_30: 0,
    days_31_60: 0,
    days_61_90: 0,
...
const statusConfig: Record<string, { label: string; cls: string }> = {
  ...
  irrecoverable: {
    label: "Irrecouv.",
    cls: "text-gray-500 bg-gray-100 line-through",
  },
...
export function AgedReceivablesReport() {
  ...
  const [expandedZones, setExpandedZones] = useState<Set<string>>(
    new Set(["Zone A", "Zone B", "Contentieux", "Autres"]),
  );
  ...
  const summary = useMemo(() => {
    ...
    const recoverable = MOCK_RECEIVABLES.reduce(
      (s, r) => s + (r.total * r.probability) / 100,
      0,
    );
    const lateCount = MOCK_RECEIVABLES.filter(
      (r) => r.status !== "normal" && r.status !== "vacant",
    ).length;
  ...
  const top5 = [...MOCK_RECEIVABLES]
    ...
    .sort(
      (a, b) =>
        ...
        b.days_61_90 +
        b.over_90 -
        (a.days_1_30 + a.days_31_60 + a.days_61_90 + a.over_90),
    )
    .slice(0, 5)
  ...
  return (
    <div className="space-y-4">
        ...
        <Card>
          <CardContent className="p-3">
            ...
            <p className="text-lg font-bold text-green-600">
              {fmtPct(summary.healthy, summary.total)}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            ...
            <p className="text-lg font-bold text-orange-500">
              {fmtPct(summary.atRisk, summary.total)}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            ...
            <p className="text-lg font-bold text-red-600">
              {fmtPct(summary.critical, summary.total)}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-3">
            ...
            <p className="text-lg font-bold">
              38 jours{" "}
              <span className="text-xs text-orange-500">+8 vs budget</span>
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Repartition par tranche</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Evolution DSO (12 mois)</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Top 5 debiteurs en retard</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Par zone et par locataire — montants en FCFA
          </CardDescription>
        </CardHeader>
          ...
          <Table>
            ...
            <TableBody>
                  ...
                  <>
                    ...
                    >
                      ...
                      <TableCell className="text-right text-xs font-medium">
                        {zoneNotDue > 0 ? (
                          <CurrencyDisplay amount={zoneNotDue} currency="XOF" />
                        ) : (
                          "—"
                        )}
                      </TableCell>
                      <TableCell className="text-right text-xs font-medium">
                        {zoneD130 > 0 ? (
                          <CurrencyDisplay amount={zoneD130} currency="XOF" />
                        ) : (
                          "—"
                        )}
                      </TableCell>
                      <TableCell className="text-right text-xs font-medium">
                        {zoneD3160 > 0 ? (
                          <CurrencyDisplay amount={zoneD3160} currency="XOF" />
                        ) : (
                          "—"
                        )}
                      </TableCell>
                      <TableCell className="text-right text-xs font-medium">
                        {zoneD6190 > 0 ? (
                          <CurrencyDisplay amount={zoneD6190} currency="XOF" />
                        ) : (
                          "—"
                        )}
                      </TableCell>
                      <TableCell className="text-right text-xs font-medium">
                        {zoneOver90 > 0 ? (
                          <CurrencyDisplay amount={zoneOver90} currency="XOF" />
                        ) : (
                          "—"
                        )}
                      </TableCell>
                      <TableCell className="text-right text-xs font-bold">
                        <CurrencyDisplay amount={zoneTotal} currency="XOF" />
                      </TableCell>
                      <TableCell />
                      <TableCell />
                    </TableRow>
                      ...
                      items.map((r) => {
                        ...
                        return (
                          <TableRow
                            ...
                            className={cn(
                              "text-xs",
                              r.status === "contentieux" &&
                                "bg-red-50/30 italic",
                              r.status === "vacant" && "text-gray-400",
                            )}
                          >
              ...
              <TableRow className="font-bold border-t-2 bg-muted/20">
                ...
                <TableCell className="text-right">
                  <CurrencyDisplay amount={summary.d1_30} currency="XOF" />
                </TableCell>
                <TableCell className="text-right">
                  <CurrencyDisplay amount={summary.d31_60} currency="XOF" />
                </TableCell>
                <TableCell className="text-right">
                  <CurrencyDisplay amount={summary.d61_90} currency="XOF" />
                </TableCell>
                <TableCell className="text-right">
                  <CurrencyDisplay amount={summary.over_90} currency="XOF" />
                </TableCell>
                <TableCell className="text-right">
                  <CurrencyDisplay amount={summary.total} currency="XOF" />
                </TableCell>
                <TableCell className="text-center text-xs">
                  {fmtPct(summary.recoverable, summary.total)}
                </TableCell>
                <TableCell />
              </TableRow>
              <TableRow className="text-xs text-muted-foreground">
                ...
                <TableCell className="text-right">
                  {fmtPct(summary.d1_30, summary.total)}
                </TableCell>
                <TableCell className="text-right">
                  {fmtPct(summary.d31_60, summary.total)}
                </TableCell>
                <TableCell className="text-right">
                  {fmtPct(summary.d61_90, summary.total)}
                </TableCell>
                <TableCell className="text-right">
                  {fmtPct(summary.over_90, summary.total)}
                </TableCell>
