...
function downloadInvoiceTemplate(type: FlowType) {
  ...
  const headers = [
    'Date facture', 'Contrepartie', 'N° Facture', 'Description',
    'Montant HT', 'Taux TVA (%)', 'Montant TVA', 'Montant TTC',
    'Categorie', 'Date echeance', 'Compte bancaire',
  ];

  const sampleRows = type === 'receipt' ? [
    ['01/03/2026', 'ZARA CI', 'FAC-V-2026-001', 'Loyer mars 2026', 2100000, 18, 378000, 2478000, 'Loyer', '01/03/2026', 'BICICI Loyers'],
    ['01/03/2026', 'CARREFOUR Market', 'FAC-V-2026-002', 'Loyer mars 2026', 3800000, 18, 684000, 4484000, 'Loyer', '01/03/2026', 'BICICI Loyers'],
    ['01/03/2026', 'Orange CI', 'FAC-V-2026-003', 'Loyer mars 2026', 4200000, 18, 756000, 4956000, 'Loyer', '01/03/2026', 'BICICI Loyers'],
    ['01/03/2026', 'Banque Atlantique', 'FAC-V-2026-004', 'Loyer mars 2026', 5500000, 18, 990000, 6490000, 'Loyer', '01/03/2026', 'BICICI Loyers'],
    ['01/03/2026', 'ZARA CI', 'FAC-V-2026-005', 'Charges mars 2026', 350000, 18, 63000, 413000, 'Charges locatives', '01/03/2026', 'BICICI Loyers'],
    ['05/03/2026', 'Parking Public', 'FAC-V-2026-010', 'Parking mars', 800000, 18, 144000, 944000, 'Revenus annexes', '05/03/2026', 'SGBCI'],
  ] : [
    ['28/02/2026', 'Prestataire Securite', 'FAC-A-2026-001', 'Gardiennage fevrier', 4200000, 18, 756000, 4956000, 'Securite', '30/03/2026', 'SGBCI'],
    ['01/03/2026', 'CIE', 'CIE-2026-03', 'Electricite fevrier', 2800000, 18, 504000, 3304000, 'Energie', '20/03/2026', 'SGBCI'],
    ['01/03/2026', 'SODECI', 'SOD-2026-03', 'Eau fevrier', 1100000, 18, 198000, 1298000, 'Energie', '20/03/2026', 'SGBCI'],
    ['05/03/2026', 'Nettoyage Pro', 'NP-2026-03', 'Nettoyage mars', 1800000, 18, 324000, 2124000, 'Nettoyage', '04/04/2026', 'SGBCI'],
    ['10/03/2026', 'Cabinet Conseil X', 'CC-2026-03', 'Honoraires mars', 3500000, 18, 630000, 4130000, 'Honoraires', '09/04/2026', 'SGBCI'],
    ['15/03/2026', 'Assurance AXA', 'AXA-2026-T1', 'Prime Q1 2026', 8400000, 0, 0, 8400000, 'Assurances', '15/03/2026', 'UBA'],
  ];
  ...
  const ws = XLSX.utils.aoa_to_sheet(data);
  ws['!cols'] = [14, 22, 18, 28, 14, 12, 14, 14, 18, 14, 18].map(w => ({ wch: w }));
  XLSX.utils.book_append_sheet(wb, ws, type === 'receipt' ? 'Factures vente' : 'Factures achat');

  // Categories sheet
  const cats = type === 'receipt'
    ? [['Loyer'], ['Charges locatives'], ['Revenus annexes'], ['Pas de porte'], ['Parking'], ['Indemnites'], ['Autres revenus']]
    : [['Securite'], ['Energie'], ['Nettoyage'], ['Maintenance'], ['Honoraires'], ['Assurances'], ['Marketing'], ['Frais generaux'], ['CAPEX'], ['Salaires'], ['Taxes']];

  const catWs = XLSX.utils.aoa_to_sheet([['Categories disponibles'], ...cats]);
  XLSX.utils.book_append_sheet(wb, catWs, 'Categories');

  XLSX.writeFile(wb, `CashPilot_Template_${type === 'receipt' ? 'Factures_Vente' : 'Factures_Achat'}.xlsx`);
}
    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 0; i < dataRows.length; i++) {
          ...
          const amountHT = parseFloat(String(row[4] || '0').replace(/\s/g, '').replace(',', '.')) || 0;
          const tvaRate = parseFloat(String(row[5] || '18').replace(',', '.')) || 0;
          const tvaAmount = parseFloat(String(row[6] || '0').replace(/\s/g, '').replace(',', '.')) || Math.round(amountHT * tvaRate / 100);
          const amountTTC = parseFloat(String(row[7] || '0').replace(/\s/g, '').replace(',', '.')) || (amountHT + tvaAmount);
          ...
          if (!isoDate) { status = 'error'; error = 'Date invalide'; }
          else if (!counterparty) { status = 'error'; error = 'Contrepartie manquante'; }
          else if (amountHT <= 0 && amountTTC <= 0) { status = 'error'; error = 'Montant invalide'; }
          else if (!category) { status = 'warning'; error = 'Categorie manquante (sera classee "Autre")'; }
...
export function BulkImport({ open, onOpenChange, defaultType = 'receipt' }: BulkImportProps) {
  ...
  const stats = useMemo(() => {
    ...
    const totalHT = invoices.filter(i => i.status !== 'error').reduce((s, i) => s + i.amount_ht, 0);
    const totalTVA = invoices.filter(i => i.status !== 'error').reduce((s, i) => s + i.tva_amount, 0);
    const totalTTC = invoices.filter(i => i.status !== 'error').reduce((s, i) => s + i.amount_ttc, 0);
    return { valid, warnings, errors, total: invoices.length, totalHT, totalTVA, totalTTC };
  }, [invoices]);
  ...
  function fmtFCFA(centimes: number) {
    return new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(centimes / 100);
  }
      ...
      <DialogContent className="sm:max-w-[900px] max-h-[85vh] overflow-y-auto">
        ...
        {step === 'upload' && (
          <div className="space-y-4">
            ...
            {/* File upload */}
            <div>
              <p className="text-sm font-medium mb-2">2. Importez votre fichier rempli</p>
              <FileUpload
                accept=".xlsx,.xls,.csv"
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-3">
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground">Factures lues</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-3 text-center">
            ...
            <div className="flex items-center gap-4 text-sm bg-muted/30 rounded-lg p-3">
              ...
              <div>
                ...
                <span className="font-medium">{fmtFCFA(stats.totalTVA)} FCFA</span>
              </div>
              <div>
                <span className="text-muted-foreground">Total TTC : </span>
                <span className="font-bold">{fmtFCFA(stats.totalTTC)} FCFA</span>
              </div>
              <Badge variant="outline" className="ml-auto text-xs">{file?.name}</Badge>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {invoices.map((inv) => (
                    ...
                    >
                      <TableCell className="text-muted-foreground">{inv.row}</TableCell>
                      <TableCell>{inv.date}</TableCell>
                      <TableCell className="font-medium max-w-[120px] truncate">{inv.counterparty}</TableCell>
                      <TableCell className="text-muted-foreground">{inv.reference}</TableCell>
                      <TableCell className="max-w-[150px] truncate">{inv.description}</TableCell>
                      <TableCell className="text-right tabular-nums">{fmtFCFA(inv.amount_ht)}</TableCell>
                      <TableCell className="text-right tabular-nums text-muted-foreground">{fmtFCFA(inv.tva_amount)}</TableCell>
                      <TableCell className="text-right tabular-nums font-medium">{fmtFCFA(inv.amount_ttc)}</TableCell>
                      <TableCell><Badge variant="outline" className="text-[9px]">{inv.category}</Badge></TableCell>
                      <TableCell className="text-center">
                        {inv.status === 'valid' && <CheckCircle2 className="h-4 w-4 text-green-500 mx-auto" />}
                        {inv.status === 'warning' && (
                          <span title={inv.error}><AlertTriangle className="h-4 w-4 text-yellow-500 mx-auto" /></span>
                        )}
                        {inv.status === 'error' && (
                          <span title={inv.error}><XCircle className="h-4 w-4 text-red-500 mx-auto" /></span>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            {stats.errors > 0 && (
              <div className="bg-red-50 border border-red-200 rounded p-3">
                <p className="text-xs font-medium text-red-800 mb-1">{stats.errors} ligne(s) en erreur — elles seront ignorees a l'import :</p>
                {invoices.filter(i => i.status === 'error').slice(0, 5).map(i => (
                  <p key={i.row} className="text-xs text-red-600">Ligne {i.row} : {i.error}</p>
                ))}
              </div>
            )}
          </div>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="p-3 rounded-lg bg-green-50">
                  <p className="text-2xl font-bold text-green-600">{stats.valid + stats.warnings}</p>
                  <p className="text-xs text-muted-foreground">Factures importees</p>
                </div>
                <div className="p-3 rounded-lg bg-red-50">
                  <p className="text-2xl font-bold text-red-600">{stats.errors}</p>
                  <p className="text-xs text-muted-foreground">Ignorees (erreurs)</p>
                </div>
                <div className="p-3 rounded-lg bg-blue-50">
                  <p className="text-2xl font-bold text-blue-600">{fmtFCFA(stats.totalTTC)}</p>
                  <p className="text-xs text-muted-foreground">Total TTC importe</p>
                </div>
              </div>
              <p className="text-xs text-muted-foreground">
        ...
        <DialogFooter>
          ...
          {step === 'result' && (
            <>
              <Button variant="outline" onClick={handleReset}>Importer un autre fichier</Button>
