...
function buildWaterfallData(
  ...
  decomposition.causes.forEach((cause) => {
    ...
    bars.push({
      name:
        cause.label.length > 30
          ? cause.label.substring(0, 28) + "..."
          : cause.label,
      value: cause.amount,
  ...
  return (
    <div>
      ...
      >
        ...
        <span
          className={`font-bold text-sm min-w-[100px] text-right ${cause.amount < 0 ? "text-red-600" : "text-green-600"}`}
        >
          {formatFrancs(cause.amount, { compact: true })}
        </span>
...
export function CausalDecompositionViz({
  decomposition,
}: CausalDecompositionVizProps) {
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">
            Decomposition causale - {decomposition.period}
          </CardTitle>
          <CardDescription>
            Variation totale:{" "}
            <strong className="text-red-600">
              {formatFrancs(decomposition.total_variation, { compact: true })}
            </strong>{" "}
            — Residuel non explique: {decomposition.residual_pct}%
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            >
              ...
              <Tooltip
                formatter={(value: number) => [
                  formatFrancs(value, { compact: true }),
                  "Montant",
                ]}
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Cliquez sur une cause pour voir la chaine de causes racines
          </CardDescription>
        </CardHeader>
          ...
          <div className="divide-y">
            ...
            >
              ...
              <span className="flex-1 font-medium text-muted-foreground">
                Residuel (non explique)
              </span>
              <Badge variant="secondary" className="text-xs">
                Residuel
              </Badge>
              <span className="font-bold text-sm min-w-[100px] text-right text-muted-foreground">
                {formatFrancs(decomposition.residual_amount, { compact: true })}
              </span>
              <span className="text-xs text-muted-foreground min-w-[40px] text-right">
                {decomposition.residual_pct}%
              </span>
              <span className="text-xs text-muted-foreground min-w-[60px] text-right" />
            </div>
          </div>
