  ...
  const chartData = useMemo(
    () =>
      distribution.density_points.map((p) => ({
        ...
        y: p.y,
        inCI80:
          p.x >= distribution.credible_interval_80[0] &&
          p.x <= distribution.credible_interval_80[1]
            ? p.y
            : 0,
      })),
    [distribution],
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Densite de la prevision pour la date selectionnee
            </CardDescription>
          </div>
          ...
          >
            ...
            <Tooltip
              formatter={(value: number, name: string) => {
                if (name === "inCI80")
                  return [value.toExponential(3), "IC 80%"];
                return [value.toExponential(3), "Densite"];
              }}
              labelFormatter={(label: number) => formatFrancs(label)}
            />
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-xs text-muted-foreground">
          <span>
            ...
            <strong className="text-foreground">
              {formatFrancs(distribution.central)}
            </strong>
          </span>
          <span>
            Ecart-type:{" "}
            <strong className="text-foreground">
              {formatFrancs(distribution.std_dev)}
            </strong>
          </span>
          <span>
            IC 80%:{" "}
            <strong className="text-foreground">
              {formatFrancs(distribution.credible_interval_80[0])} -{" "}
              {formatFrancs(distribution.credible_interval_80[1])}
            </strong>
          </span>
          <span>
            MC Dropout:{" "}
            <strong className="text-foreground">
              {distribution.mc_dropout_samples} passes
            </strong>
          </span>
        </div>
      </CardContent>
