...
const initialAlerts: FraudAlert[] = [
  {
    id: "1",
    date: "20/03/2026 14:22",
    type: "duplicate_payment",
    severity: "critical",
    status: "detected",
    description:
      "Doublon detecte: virement de 8 500 000 FCFA a SAHAM Assurance identique au paiement du 13/03/2026 (meme montant, meme beneficiaire, meme reference)",
    amount: 8_500_000,
    evidence: [
    ...
    recommendedActions: [
      "Bloquer immediatement le second virement",
      "Verifier avec le fournisseur si deux factures distinctes existent",
      "Auditer l'historique des paiements a SAHAM sur 6 mois",
    ],
  },
  {
    id: "2",
    date: "19/03/2026 23:47",
    type: "timing_anomaly",
    severity: "critical",
    status: "investigating",
    description:
      "Transaction de 25 000 000 FCFA initiee a 23h47 depuis le compte SGBCI Exploitation vers un compte externe. Hors plage horaire autorisee (08h-18h).",
    amount: 25_000_000,
    evidence: [
    ...
    recommendedActions: [
      "Geler le compte beneficiaire immediatement",
      "Revoquer le token API #TK-882",
      "Contacter le responsable IT pour analyse des logs",
      "Signaler a la Banque pour rappel de fonds si non encore credite",
    ],
  },
  {
    id: "3",
    ...
    status: "detected",
    description:
      "Paiement fournisseur BATIREX SARL de 45 200 000 FCFA. Montant 3.2x superieur a la moyenne historique des paiements a ce fournisseur (14 100 000 FCFA).",
    amount: 45_200_000,
    evidence: [
    ...
    recommendedActions: [
      "Verifier le bon de commande BC-2026-0234 et obtenir validation de la direction",
      "Comparer avec le contrat cadre BATIREX",
      "Demander une facture detaillee avant reglement",
    ],
  },
  {
    id: "4",
    date: "17/03/2026 09:30",
    type: "unauthorized_vendor",
    severity: "high",
    status: "detected",
    description:
      "Ordre de paiement de 12 000 000 FCFA au profit de GLOBAL SERVICES INTL (non reference dans la base fournisseurs approuves).",
    amount: 12_000_000,
    evidence: [
    ...
    recommendedActions: [
      "Suspendre l'ordre de paiement",
      "Verifier l'identite et l'existence legale de GLOBAL SERVICES INTL",
      "Exiger un contrat signe et un bon de commande valide",
      "Verifier les autorisations de S. Traore",
    ],
  },
  {
    id: "5",
    date: "16/03/2026 15:20",
    type: "round_amount",
    severity: "medium",
    status: "dismissed",
    description:
      "Serie de 4 virements de 5 000 000 FCFA chacun (total: 20 000 000 FCFA) en 15 minutes vers 4 comptes differents. Pattern de fractionnement potentiel.",
    amount: 20_000_000,
    evidence: [
    ...
    recommendedActions: [
      "Verifier si les 4 beneficiaires sont lies",
      "Croiser avec les bons de commande",
      "Alerter le responsable conformite",
    ],
  },
  {
    id: "6",
    ...
    status: "confirmed",
    description:
      "Double paiement salaire employe #EMP-0234 (A. Kone) pour le mois de fevrier 2026. Montant: 1 850 000 FCFA x 2.",
    amount: 1_850_000,
    evidence: [
    ...
    recommendedActions: [
      "Contacter l'employe pour recuperation du trop-percu",
      "Verifier le processus de paie complementaire",
      "Mettre a jour le controle des doublons",
    ],
  },
];
...
export default function Fraud() {
  ...
  // Explanation state
  const [explanationAlertId, setExplanationAlertId] = useState<string | null>(
    null,
  );
  const [explanationData, setExplanationData] =
    useState<AnomalyExplanation | null>(null);
  ...
  const handleExplanationFeedback = (
    anomalyId: string,
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6">
      <PageHeader
        ...
        description="Detection temps reel des patterns de fraude par le moteur IA"
      />
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-medium whitespace-nowrap">
                        {formatFrancs(alert.amount)}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${sev.badgeCls}`}
                        >
                          {sev.label}
                        </span>
                      </td>
            ...
            <>
              ...
              <div className="space-y-4 mt-2">
                <div className="flex items-center gap-3">
                  <span
                    className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${severityConfig[selectedAlert.severity].badgeCls}`}
                  >
                  ...
                  <span className="text-sm font-bold">
                    {formatFrancs(selectedAlert.amount)}
                  </span>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {selectedAlert.evidence.map((e, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 bg-muted/50 rounded px-3 py-2 text-sm"
                      >
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {selectedAlert.recommendedActions.map((a, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 bg-blue-50 dark:bg-blue-950/20 rounded px-3 py-2 text-sm"
                      >
                ...
                {(selectedAlert.status === "detected" ||
                  ...
                  <>
                    <Button
                      variant="outline"
                      onClick={() =>
                        handleStatusChange(selectedAlert.id, "dismissed")
                      }
                    >
                    ...
                    <Button
                      variant="destructive"
                      onClick={() =>
                        handleStatusChange(selectedAlert.id, "confirmed")
                      }
                    >
