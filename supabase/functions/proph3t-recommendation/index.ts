...
import { sum as _sum } from "../_shared/statistics.ts";
import {
  toISODate as _toISODate,
  today as _today,
...
function generateInternalTransfer(
  ...
  const sorted = [...accounts].sort(
    (a, b) =>
      ((b.current_balance as number) || 0) -
      ((a.current_balance as number) || 0),
  );
...
function generateCreditLineActivation(
  ...
  const bestLine = available.reduce((best, cl) => {
    const bestAvail =
      ((best.credit_limit as number) || 0) -
      ((best.used_amount as number) || 0);
    const clAvail =
      ((cl.credit_limit as number) || 0) - ((cl.used_amount as number) || 0);
    return clAvail > bestAvail ? cl : best;
  });

  const availAmount =
    ((bestLine.credit_limit as number) || 0) -
    ((bestLine.used_amount as number) || 0);
...
function generatePriorityTenantFollowup(
  ...
  const counterparties =
    (details.counterparties as Record<string, unknown>[]) || [];
  const critical = counterparties.filter(
    (c) => (c.classification as string) === "critical",
  );
...
function scoreRemediation(option: RemediationOption): number {
  ...
  const costScore =
    option.cost === 0
      ? 1.0
      : Math.max(0, 1 - option.cost / Math.max(1, option.amount));
  const speedScore = Math.max(0, 1 - option.delay_days / 30);
  const coverageScore =
    option.amount > 0 ? Math.min(1, option.amount / 10_000_000_00) : 0.3; // normalize to 10M
...
async function executeRecommendation(
  ...
  const creditLine = generateCreditLineActivation(
    input.alert_details,
    creditLines,
  );
  ...
  try {
    ...
    if (!body.alert_id || !body.tenant_id || !body.company_id) {
      return errorResponse(
        "Missing required fields: alert_id, tenant_id, company_id",
        400,
      );
    }
