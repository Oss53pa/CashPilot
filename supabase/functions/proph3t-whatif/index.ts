...
import { mean as _mean, sum, min as _min } from "../_shared/statistics.ts";
import {
  toISODate,
  today,
  addDays,
  daysDiff as _daysDiff,
...
function parseQuery(query: string, _lang: "fr" | "en"): ParsedQuery {
  ...
  const patterns: {
    ...
    },
    {
      ...
      patterns: [/ligne\s+de\s+crédit/i, /credit\s+line/i, /tirage|drawdown/i],
      confidence: 0.75,
    },
    {
      intent: "scenario_comparison",
      patterns: [/compar(?:er|aison|e)\s+.*scénario/i, /compare\s+.*scenario/i],
      confidence: 0.7,
    },
    {
      intent: "days_of_cash",
...
async function runSimulation(
  ...
  const totalBalance = sum(
    accounts.map((a) => (a.current_balance as number) || 0),
  );
  ...
  for (const f of forecasts) {
    ...
    forecastByDate.set(
      date,
      (forecastByDate.get(date) || 0) + (f.amount_central as number),
    );
  }
  ...
  switch (parsed.intent) {
    case "position_if_no_payment": {
      ...
      const removedAmount =
        sum(cpForecasts.map((f) => (f.amount_central as number) || 0)) * 0.3; // estimate 30%
    ...
    case "max_capex_budget": {
      ...
      const safetyMargin = totalBalance * 0.1;
    ...
    case "days_of_cash": {
      ...
      const totalDisb = sum(
        disbursements.map((d) => Math.abs((d.amount as number) || 0)),
      );
      const avgDaily = totalDisb / 30;
      const daysOfCash =
        avgDaily > 0 ? Math.round(totalBalance / avgDaily) : 999;
    ...
    case "stress_test": {
      ...
      const totalExpected = sum(
        receivableForecasts.map((f) => (f.amount_central as number) || 0),
      );
...
async function executeWhatIf(
  ...
  // Run simulation
  const { result, narrative } = await runSimulation(
    supabase,
    ctx,
    parsed,
    language,
