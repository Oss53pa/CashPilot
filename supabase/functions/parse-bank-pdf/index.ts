        ...
        while (j < streamContent.length) {
          if (streamContent[j] === "(") {
            ...
            while (j < streamContent.length && depth > 0) {
              if (streamContent[j] === "(" && streamContent[j - 1] !== "\\")
                depth++;
              else if (
                streamContent[j] === ")" &&
                streamContent[j - 1] !== "\\"
...
function parseTransactionLines(text: string): ParsedTransaction[] {
  ...
  for (const line of lines) {
    ...
    const isDebit = /debit|dbt|sortie|prlv|vir\s*em|paiement|retrait/i.test(
      line,
    );
    const isCredit = /credit|cdt|entree|versement|remise|encaissement/i.test(
      line,
    );
    ...
    const refMatch = description.match(
      /(?:REF|N°|NO|NUM)\s*:?\s*([A-Z0-9\-/]+)/i,
    );
    const reference = refMatch ? refMatch[1] : null;

    // Try to extract counterparty (capitalized words)
    const cpMatch = description.match(
      /(?:DE|A|VERS|FROM|TO)\s+([A-Z][A-Z\s]{3,})/i,
    );
...
async function executePdfParse(
  ...
  if (input.file_type === "pdf") {
    ...
    if (!rawText || rawText.length < 50) {
      ...
      if (ocr.method === "manual_required") {
        return {
          status: "ocr_required",
          message:
            "Ce PDF semble etre une image scannee. Le fichier a ete stocke pour traitement manuel.",
    ...
    if (ocr.method === "manual_required") {
      return {
        status: "ocr_required",
        message:
          "L'image a ete stockee pour traitement. Configurez un service OCR pour l'extraction automatique.",
        file_name: input.file_name,
        extraction_method: extractionMethod,
      };
    }
  ...
  if (transactions.length === 0) {
    return {
      ...
      message:
        "Aucune transaction n'a pu etre extraite du fichier. Le texte extrait est peut-etre insuffisant ou dans un format non reconnu.",
      file_name: input.file_name,
      extraction_method: extractionMethod,
  ...
  try {
    ...
    if (!body.account_id || !body.file_content || !body.file_type) {
      return errorResponse(
        "Missing required fields: account_id, file_content, file_type",
        400,
      );
    }
