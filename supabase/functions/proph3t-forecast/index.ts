...
import {
  ...
  toISODate,
  today,
  monthsDiff as _monthsDiff,
  groupByMonth as _groupByMonth,
...
function weightedMovingAverage(
  ...
  const wma =
    lastWindow.reduce((sum, val, i) => sum + val * activeWeights[i], 0) /
    totalWeight;
  ...
  for (let i = windowSize; i < series.length; i++) {
    ...
    const pred =
      window.reduce((sum, val, j) => sum + val * activeWeights[j], 0) /
      totalWeight;
    residuals.push(series[i] - pred);
  }

  const residualStd =
    residuals.length > 0 ? stddev(residuals) : stddev(series) * 0.1;
...
function hwFitError(
  ...
  let trend =
    (mean(series.slice(period, period * 2)) - mean(series.slice(0, period))) /
    period;
  let totalError = 0;
  ...
  for (let i = period; i < n; i++) {
    ...
    totalError +=
      Math.abs(series[i] - predicted) / Math.max(1, Math.abs(series[i]));
...
function arimaForecast(
  ...
  let _bestP = 1,
    _bestQ = 1;
  let bestCoeffs: { ar: number[]; ma: number[]; intercept: number } | null =
    null;
  ...
  const diffPredictions = forecastARMA(
    diffSeries,
    bestCoeffs,
    bestResiduals,
    forecastHorizon,
  ...
  const residualStd =
    bestResiduals.length > 0 ? stddev(bestResiduals) : stddev(series) * 0.1;
...
function sarimaForecast(
  ...
  const { predictions: diffPredictions, residualStd: baseStd } = arimaForecast(
    seasonDiff,
    forecastHorizon,
  );
  ...
  for (let h = 0; h < forecastHorizon; h++) {
    ...
    const seasonalBase =
      seasonIdx >= 0 && seasonIdx < n ? series[seasonIdx] : series[n - 1];
...
async function loadCashFlowHistory(
  ...
  for (const flow of flows) {
    ...
    const existing = byCategory
      .get(cat)!
      .find((p) => p.date.startsWith(monthKey));
    if (existing) {
      ...
      byCategory
        .get(cat)!
        .push({
          date: flow.flow_date.substring(0, 7) + "-01",
          amount: flow.amount,
        });
    }
  }
...
function prophetForecast(
  ...
  for (let i = 0; i < seasonPeriod; i++) {
    seasonalPattern[i] =
      seasonalCounts[i] > 0 ? seasonalPattern[i] / seasonalCounts[i] : 0;
  }
...
function ensembleForecast(
  ...
  const models: {
    name: string;
    result: { predictions: number[]; residualStd: number };
    mape: number;
  }[] = [];

  const modelsToTry: {
    name: string;
    fn: () => { predictions: number[]; residualStd: number };
  }[] = [
  ...
  if (series.length >= 24) {
    modelsToTry.push({
      name: "lstm",
      fn: () => lstmForecast(series, forecastHorizon),
    });
  }
  ...
  for (let h = 0; h < forecastHorizon; h++) {
    ...
    for (let i = 0; i < top.length; i++) {
      const p =
        h < top[i].result.predictions.length
          ? top[i].result.predictions[h]
          : top[i].result.predictions[top[i].result.predictions.length - 1];
      pred += weights[i] * p;
    }
    predictions.push(Math.round(pred));
  }

  const avgResidualStd = top.reduce(
    (s, m, i) => s + weights[i] * m.result.residualStd,
    0,
  );
...
function simpleBacktest(
  ...
  const numFolds = Math.min(
    3,
    Math.floor((values.length - minTrainSize) / foldSize),
  );
...
async function executeForecast(
  ...
  for (const { category, series } of categoryData) {
    ...
    const { mapeScore, _maeScore, _rmseScore } = simpleBacktest(
      values,
      modelType,
    );
    ...
    const { predictions, residualStd } = runModel(
      modelType,
      values,
      maxHorizon,
    );
