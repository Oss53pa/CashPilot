...
import { mean as _mean, sum } from "../_shared/statistics.ts";
import {
  daysDiff,
  toISODate,
  today,
...
async function loadTransactionContext(
  ...
  const knownIBANs = counterparties
    .map(
      (c: Record<string, unknown>) =>
        (c.iban || c.bank_account_number) as string,
    )
    .filter(Boolean);
  ...
  const velocity = computeVelocity(
    recentPayments
      .filter(
        (p: Record<string, unknown>) => p.counterparty_id === counterpartyId,
      )
...
function checkPaymentSmurfing(ctx: TransactionContext): RuleResult {
  ...
  const subThresholdIn48h = ctx.recentPayments.filter(
    (p: Record<string, unknown>) => {
      if (p.counterparty_id !== counterpartyId) return false;
      const diff = daysDiff(
        p.flow_date as string,
        ctx.transaction.flow_date as string,
      );
  ...
  const totalAmount =
    sum(
      subThresholdIn48h.map((p: Record<string, unknown>) => p.amount as number),
    ) + amount;
  const triggered =
    subThresholdIn48h.length >= 2 && totalAmount >= DOA_THRESHOLD;
...
function checkGhostVendor(ctx: TransactionContext): RuleResult {
  ...
  const triggered =
    !counterpartyId ||
    isUnknown ||
    (isNew && ctx.counterpartyProfile.transaction_count <= 1);
  const severity: FraudSeverity = !counterpartyId
    ? "critical"
    : isUnknown
      ? "high"
      : "medium";
...
function checkDuplicatePayment(ctx: TransactionContext): RuleResult {
  ...
  const duplicates = ctx.recentPayments.filter((p: Record<string, unknown>) => {
    ...
    return (
      diff >= 0 &&
      diff <= 30 &&
      Math.abs((p.amount as number) - amount) < amount * 0.01
    );
  });
...
function checkRibChangeThenPayment(ctx: TransactionContext): RuleResult {
  ...
  const relevantChanges = ctx.ribChanges.filter(
    (c: Record<string, unknown>) => {
...
function checkCashRegisterManipulation(ctx: TransactionContext): RuleResult {
  ...
  const cashDiscrepancies = ctx.recentPayments.filter(
    (p: Record<string, unknown>) => {
      const diff = daysDiff(p.flow_date as string, flowDate);
      return (
        diff >= 0 &&
  ...
  return {
    ...
    evidence: triggered
      ? {
          ...
          total_amount: sum(
            cashDiscrepancies.map((p: Record<string, unknown>) =>
              Math.abs(p.amount as number),
            ),
          ),
...
function checkUnauthorizedTransfer(ctx: TransactionContext): RuleResult {
  ...
  const approvedBy =
    (ctx.transaction.approved_by as string) ||
    (ctx.transaction.validated_by as string) ||
    null;

  const triggered =
    Math.abs(amount) >= APPROVAL_THRESHOLD &&
    !approvedBy &&
    status !== "approved";
  ...
  return {
    ...
    score: triggered
      ? Math.min(
          95,
          60 + Math.floor(Math.abs(amount) / APPROVAL_THRESHOLD) * 10,
        )
      : 0,
    message: triggered
...
function checkOffHoursTransaction(ctx: TransactionContext): RuleResult {
  ...
  return {
    ...
    score: triggered
      ? (isOffHours ? 60 : 40) + (isHoliday ? 15 : 0) + (isWeekend ? 10 : 0)
      : 0,
    message: triggered
      ? isOffHours
...
function checkExcessCashNotDeposited(ctx: TransactionContext): RuleResult {
  ...
  const cashIn7d = ctx.recentPayments.filter((p: Record<string, unknown>) => {
    ...
    return (
      diff >= 0 && diff <= 7 && (cat === "cash_register" || cat === "cash")
    );
  });

  const cashBalance = sum(
    cashIn7d.map((p: Record<string, unknown>) => p.amount as number),
  );
  ...
  return {
    ...
    score: triggered
      ? Math.min(85, 50 + Math.floor(cashBalance / CASH_THRESHOLD) * 15)
      : 0,
    message: triggered
...
async function executeFraudCheck(
  ...
  for (const rule of triggered) {
    ...
    if (error) {
      console.error(
        `Failed to insert fraud alert for rule ${rule.rule_id}:`,
        error,
      );
    }
  }
      ...
      for (const company of companies ?? []) {
        try {
          ...
          for (const flow of recentFlows ?? []) {
            try {
              const result = await executeFraudCheck(supabase, ctx, {
