...
import {
  ...
  toISODate,
  today,
  cyclicalEncode as _cyclicalEncode,
} from "../_shared/date-utils.ts";
import {
  buildCounterpartyProfile,
  computeVelocity as _computeVelocity,
  computeZScoreFeatures as _computeZScoreFeatures,
  hourNormality,
  sequentialCount,
...
function isolationForestScore(
  ...
  const maxDepth = Math.ceil(
    Math.log2(Math.min(sampleSize, historicalData.length)),
  );
  ...
  const avgPath =
    trees.reduce((sum, tree) => sum + pathLength(point, tree, 0), 0) / numTrees;
...
async function extractFeatures(
  ...
  historicalFeatureVectors: number[][];
}> {
  ...
  const { data: history = [] } = await supabase
    .from("cash_flows")
    .select(
      "id, flow_date, amount, category, counterparty_id, account_id, created_at",
    )
  ...
  const histAmounts = history.map(
    (h: Record<string, unknown>) => h.amount as number,
  );

  // Feature 1: Amount Z-score
  const amountZScore =
    histAmounts.length >= 3 ? zScore(amount, histAmounts) : 0;

  // Feature 2: Timing deviation (days since last similar transaction)
  const lastSimilar = history.filter(
    (h: Record<string, unknown>) => h.counterparty_id === counterpartyId,
  );
  ...
  if (lastSimilar.length >= 2) {
    ...
    if (stdInterval > 0 && lastSimilar.length > 0) {
      const daysSinceLast = daysDiff(
        (lastSimilar[lastSimilar.length - 1] as Record<string, unknown>)
          .flow_date as string,
        flowDate,
      );
  ...
  const accountTxns = history.filter(
    (h: Record<string, unknown>) => h.account_id === accountId,
  );
  const accountFrequency =
    history.length > 0 ? 1 - accountTxns.length / history.length : 0.5;
  ...
  const amountVsBudget =
    categoryMean !== 0 ? Math.abs(amount / categoryMean - 1) : 0;

  // Feature 7: Sequential count (same amount repeated)
  const seqCount =
    sequentialCount(histAmounts, amount) / Math.max(1, histAmounts.length);
  ...
  const historicalFeatureVectors: number[][] = history.map(
    (h: Record<string, unknown>) => {
      const hAmount = h.amount as number;
      const hZScore =
        histAmounts.length >= 3 ? zScore(hAmount, histAmounts) : 0;
      const hAccountTxns = history.filter(
        (hh: Record<string, unknown>) => hh.account_id === h.account_id,
      );
      const hAccountFreq =
        history.length > 0 ? 1 - hAccountTxns.length / history.length : 0.5;
      const hCounterpartyKnown = history.some(
      ...
      const hBudgetRatio =
        categoryMean !== 0 ? Math.abs(hAmount / categoryMean - 1) : 0;
...
function generateExplanation(
  ...
  const contributions: FeatureContribution[] = [
    ...
    },
    {
      ...
      description:
        features.account_frequency > 0.7
...
async function executeAnomalyScan(
  ...
  if (!cashFlow) {
    return {
      status: "skipped",
      message: "No cash flow or payment request found",
    };
  }
  ...
  const anomalyScore = isolationForestScore(
  ...
  const { human_readable, top_reasons } = generateExplanation(
    features,
    severity,
  );
      ...
      for (const company of companies ?? []) {
        try {
          ...
          for (const flow of recentFlows ?? []) {
            try {
              const result = await executeAnomalyScan(supabase, ctx, {
