...
import { toISODate, today as _today } from "../_shared/date-utils.ts";
...
async function walkForwardBacktest(
  ...
  if (flows.length < 12) {
    return {
      ...
      message: "Need at least 12 months of history",
    };
  }
  ...
  for (const flow of flows) {
    ...
    const _monthKey = (flow.flow_date as string).substring(0, 7);
  ...
  for (const [category, values] of byCategory.entries()) {
    ...
    const numFolds = Math.min(
      6,
      Math.floor((values.length - minTrain) / foldSize),
    );
    ...
    const modelScores: Record<
      string,
      { mapeScores: number[]; maeScores: number[]; rmseScores: number[] }
    > = {};
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const _ctx = {
      tenant_id: body.scope || "",
      company_id: "",
    } as TenantContext;
    // For manual invocation, use extractContext
    const ctxReal = await (
      await import("../_shared/supabase-client.ts")
    ).extractContext(req, supabase, body as Record<string, unknown>);
