...
import { mean as _mean, sum, mape } from "../_shared/statistics.ts";
...
async function loadNarrativeData(supabase: SupabaseClient, ctx: TenantContext) {
  ...
  const [forecasts, alerts, scores, anomalies, cashFlows, accounts, perfLogs] =
    await Promise.all([
      supabase
...
function generateSections(
  ...
  const totalBalance = sum(
    data.accounts.map((a) => (a.current_balance as number) || 0),
  );
  const totalReceipts = sum(
    data.cashFlows
      .filter((f) => (f.flow_type as string) === "receipt")
      .map((f) => (f.amount as number) || 0),
  );
  const totalDisbursements = sum(
    data.cashFlows
      .filter((f) => (f.flow_type as string) === "disbursement")
      .map((f) => Math.abs((f.amount as number) || 0)),
  );

  // 1. Situation Générale
  const balanceSentiment: Sentiment =
    totalBalance > 0 ? "positive" : "critical";
  sections.push({
    ...
    data_points: [
      {
        label: isFr ? "Position globale" : "Global position",
        value: formatFCFA(totalBalance),
      },
      {
        label: isFr ? "Encaissements" : "Receipts",
        value: formatFCFA(totalReceipts),
      },
      {
        label: isFr ? "Décaissements" : "Disbursements",
        value: formatFCFA(totalDisbursements),
      },
    ],
  });
  ...
  const criticalAlerts = data.alerts.filter(
    (a) => (a.priority as string) === "critical",
  );
  const highAlerts = data.alerts.filter(
    (a) => (a.priority as string) === "high",
  );
  const criticalAnomalies = data.anomalies.filter(
    (a) => (a.severity as string) === "critical",
  );

  const attentionSentiment: Sentiment =
  ...
  const alertDetails = data.alerts
    .slice(0, 3)
  ...
  sections.push({
    ...
    data_points: [
      {
        label: isFr ? "Alertes critiques" : "Critical alerts",
        value: criticalAlerts.length,
      },
      {
        label: isFr ? "Alertes hautes" : "High alerts",
        value: highAlerts.length,
      },
      {
        label: isFr ? "Anomalies critiques" : "Critical anomalies",
  ...
  // 3. Prévision Semaine
  const weekForecasts = data.forecasts.filter(
    (f) => (f.horizon as number) <= 30,
  );
  const forecastTotal = sum(
    weekForecasts.map((f) => (f.amount_central as number) || 0),
  );
  const forecastSentiment: Sentiment =
    forecastTotal >= 0
      ? "positive"
      : forecastTotal > -totalBalance
        ? "warning"
        : "critical";
  ...
  sections.push({
    ...
    data_points: [
      {
        label: isFr ? "Flux net prévu (30j)" : "Expected net flow (30d)",
        value: formatFCFA(forecastTotal),
      },
      {
        label: isFr ? "Catégories" : "Categories",
        value: new Set(weekForecasts.map((f) => f.category)).size,
      },
    ],
  });

  // 4. Bonnes Nouvelles
  const goodScores = data.scores.filter(
    (s) =>
      (s.classification as string) === "excellent" ||
      (s.classification as string) === "good",
  );
  const improvingScores = data.scores.filter(
    (s) => (s.trend as string) === "improving",
  );
  ...
  sections.push({
    ...
    sentiment:
      goodScores.length > data.scores.length * 0.5 ? "positive" : "neutral",
    data_points: [
      {
        label: isFr ? "Scores excellents/bons" : "Excellent/good scores",
        value: goodScores.length,
      },
      {
        label: isFr ? "En amélioration" : "Improving",
        value: improvingScores.length,
      },
    ],
  });

  // 5. Vigilance CAPEX
  const capexFlows = data.cashFlows.filter(
    (f) =>
      (f.category as string) === "capex" ||
      (f.category as string) === "investment",
  );
  const totalCapex = sum(
    capexFlows.map((f) => Math.abs((f.amount as number) || 0)),
  );
  ...
  sections.push({
    ...
    data_points: [
      ...
      {
        label: isFr ? "Ratio CAPEX/Trésorerie" : "CAPEX/Treasury ratio",
        value: `${Math.round(capexRatio * 100)}%`,
      },
    ],
  });

  // 6. Performance Modèle
  const realizedLogs = data.perfLogs.filter(
    (l) => l.is_realized && l.actual_amount != null,
  );
  let modelMape = 0;
  ...
  sections.push({
    ...
    content: isFr
      ? `${realizedLogs.length} prévision(s) vérifiée(s) cette semaine avec un MAPE de ${(modelMape * 100).toFixed(1)}%. ${modelMape < 0.1 ? "Les modèles fonctionnent avec une excellente précision." : modelMape < 0.2 ? "Précision acceptable, mais un recalibrage pourrait être bénéfique." : "La précision est dégradée — un réentraînement est recommandé."}`
      : `${realizedLogs.length} prediction(s) verified this week with MAPE of ${(modelMape * 100).toFixed(1)}%. ${modelMape < 0.1 ? "Models are performing with excellent accuracy." : modelMape < 0.2 ? "Acceptable accuracy, but recalibration may help." : "Accuracy is degraded — retraining recommended."}`,
    sentiment:
      modelMape < 0.1 ? "positive" : modelMape < 0.2 ? "neutral" : "warning",
    data_points: [
      { label: "MAPE", value: `${(modelMape * 100).toFixed(1)}%` },
      {
        label: isFr ? "Prévisions vérifiées" : "Verified predictions",
        value: realizedLogs.length,
      },
    ],
  });
...
async function executeNarrative(
  ...
  const sentimentOrder: Sentiment[] = [
    "positive",
    "neutral",
    "warning",
    "critical",
  ...
  const overallSentiment = sections.reduce<Sentiment>((worst, section) => {
    return sentimentOrder.indexOf(section.sentiment) >
      sentimentOrder.indexOf(worst)
      ? section.sentiment
      : worst;
  }, "positive");
