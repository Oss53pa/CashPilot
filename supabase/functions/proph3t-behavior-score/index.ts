...
async function extractCounterpartyFeatures(
  ...
  if (payments.length >= 3) {
    ...
    paymentRegularity =
      avgInterval > 0 ? 1 - clamp(stddev(intervals) / avgInterval, 0, 1) : 0;
  }
  ...
  const lastPayment =
    payments.length > 0
      ? daysDiff(
          (payments[payments.length - 1] as Record<string, unknown>)
            .flow_date as string,
          toISODate(now),
        )
      : 999;
  ...
  return {
    ...
    partial_payment_rate:
      totalExpected > 0 ? partialPayments / totalExpected : 0,
...
function computeScore(features: CounterpartyFeatures): {
  ...
  const factors: {
  ...
  }[] = [
    {
      ...
      description:
        features.avg_delay_recent_3m > 30
    ...
    },
    {
      ...
      description:
        features.full_payment_rate < 0.7
    ...
    },
    {
      ...
      description:
        features.payment_regularity < 0.5
    ...
    },
    {
      ...
      description:
        features.days_since_last_payment > 60
  ...
  const raw_probability = factors.reduce(
    (sum, f) => sum + f.weight * f.riskContribution,
    0,
  );
  ...
  const topRisk = factors
    .sort(
      (a, b) => b.weight * b.riskContribution - a.weight * a.riskContribution,
    )
    .slice(0, 3)
    .map((f) => ({
      feature: f.feature,
  ...
  return {
    ...
    raw_probability: clamp(raw_probability, 0, 1),
    top_risk_factors: topRisk,
  };
}
...
function recommendAction(
  ...
  if (classification === "at_risk" && trend === "critical_degradation")
...
async function executeBehaviorScore(
  ...
  if (counterpartyId) {
    ...
    counterpartyIds = (counterparties ?? []).map(
  ...
  for (const cpId of counterpartyIds) {
    try {
      ...
      // Compute score
      const { score, raw_probability, top_risk_factors } =
      ...
      const previousScoreValues = prevScores.map(
        (s: Record<string, unknown>) => s.score as number,
      );
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const result = await executeBehaviorScore(
      supabase,
      ctx,
