...
import { mean as _mean, sum, min } from "../_shared/statistics.ts";
...
async function loadAlertContext(
  ...
  const [forecasts, accounts, scores, anomalies, cashFlows, creditLines] =
    await Promise.all([
      supabase
...
function checkLiquidityTension(ctx: AlertContext): AlertResult | null {
  ...
  const totalBalance = sum(
    ctx.accounts.map((a) => (a.current_balance as number) || 0),
  );
  const availableCredit = sum(
    ctx.creditLines.map((cl) => {
  ...
  const daysUntilBreach = Math.round(
    (new Date(breachDate).getTime() - today().getTime()) /
      (1000 * 60 * 60 * 24),
  );
...
function checkRecoveryRisk(ctx: AlertContext): AlertResult | null {
  ...
  const atRiskScores = ctx.scores.filter(
    (s) =>
      (s.classification as string) === "critical" ||
      (s.classification as string) === "at_risk",
  );
  ...
  const criticalCount = [...byCounterparty.values()].filter(
    (s) => (s.classification as string) === "critical",
  ).length;

  if (byCounterparty.size < 2 && criticalCount === 0) return null;

  const priority: AlertPriority =
    criticalCount >= 3 ? "critical" : criticalCount >= 1 ? "high" : "medium";
...
function checkAccountImbalance(ctx: AlertContext): AlertResult | null {
  ...
  const maxAccount = balances.reduce(
    (max, b) => (b.balance > max.balance ? b : max),
    balances[0],
  );
...
function checkCovenantBreachRisk(ctx: AlertContext): AlertResult | null {
  ...
  const totalDebt = sum(
    ctx.creditLines.map((cl) => (cl.used_amount as number) || 0),
  );
...
function checkInvestmentOpportunity(ctx: AlertContext): AlertResult | null {
  ...
  const next90 = ctx.forecasts.filter((f) => {
    const daysAway = Math.round(
      (new Date(f.forecast_date as string).getTime() - today().getTime()) /
        (1000 * 60 * 60 * 24),
    );
    return daysAway <= 90;
  });
  ...
  const minForecast = min(
    next90.map((f) => (f.amount_lower_80 as number) || 0),
  );
...
function checkCapexTension(ctx: AlertContext): AlertResult | null {
  ...
  const capexFlows = ctx.cashFlows.filter(
    (f) =>
  ...
  const _avgMonthlyCapex = totalCapex / 1; // last 30 days

  // Check forecasted CAPEX
  const capexForecasts = ctx.forecasts.filter(
    (f) => (f.category as string) === "capex",
  );
  const forecastedCapex = sum(
    capexForecasts.map((f) => Math.abs(f.amount_central as number)),
  );
...
function checkCriticalTenantScore(ctx: AlertContext): AlertResult | null {
  const criticalScores = ctx.scores.filter(
    (s) =>
      (s.classification as string) === "critical" &&
...
async function executeAlertEngine(
  ...
  for (const alert of alerts) {
    ...
    try {
      ...
      console.error(
        `Failed to generate recommendations for alert ${alert.rule_id}:`,
        err,
      );
    }
  }
